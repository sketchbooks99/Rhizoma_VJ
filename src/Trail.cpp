...
void Trail::createBuffers(int _num, int _length) {
    ...
    for(int x = 0; x < _length; x++) {
        for(int y = 0; y < _num; y++) {
            ...
            pos[index * 4 + 0] = ofRandom(-1.0, 1.0);
            pos[index * 4 + 1] = ofRandom(-1.0, 1.0);
            pos[index * 4 + 2] = ofRandom(-1.0, 1.0);
            pos[index * 4 + 3] = 0.0;
        }
    }
...
void Trail::debugTexture(int x, int y) {
    ...
    posBuffer.src->draw(x, y, length * 10, num * 0.1);
    ofDrawBitmapString("vel", x + length * 2, y - 5);
