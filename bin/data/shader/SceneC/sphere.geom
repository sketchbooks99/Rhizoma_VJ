...
// rule:
// v0 -> v1 -> v2の順で結ぶ
void createTriangle(vec3 v0, vec3 v1, vec3 v2, vec3 color) {
    ...
    vPosition = (view * model * vec4(v0, 1.0)).xyz;
    vNormal = faceNormal;
    // vColor = vertex[0].color;
    vColor = color;
    EmitVertex();
    gl_Position = mvpMatrix * vec4(v1, 1.0);
    vNormal = faceNormal;
    vPosition = (view * model * vec4(v1, 1.0)).xyz;
    vColor = color;
    EmitVertex();
    gl_Position = mvpMatrix * vec4(v2, 1.0);
    vPosition = (view * model * vec4(v2, 1.0)).xyz;
    vNormal = faceNormal;
    vColor = color;
    EmitVertex();
    EndPrimitive();
...
void main() {
    ...
    float n = cnoise(vec2(scaledUV.x * sin(time * 0.533), scaledUV.y * cos(time * 0.789))) + volume;
    float height = n;
    ...
    vec3 c = vec3(0.0);
    if (n < 0.5) {
        v0 = gl_in[0].gl_Position.xyz * (1.0-n) + center * n;
        v1 = gl_in[1].gl_Position.xyz * (1.0-n) + center * n;
        v2 = gl_in[2].gl_Position.xyz * (1.0-n) + center * n;
